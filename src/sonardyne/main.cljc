(ns sonardyne.main
  "Kotodama WASM entrypoint for the Sonardyne clean-room actor (L4) — Clojure port.
  ...
  facts (`sonardyne.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sonardyne")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sonardyne.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sonardyne-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sonardyne-compat/ready)
